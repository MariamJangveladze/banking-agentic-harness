...
from banking_harness.knowledge import FixtureKnowledgeProvider
...
def test_release_gate_requires_all_domains_and_citations() -> None:
    ...
    results = evaluate_case(
        complete_request(), findings, [], list(FixtureKnowledgeProvider.documents)
    )
...
def test_release_gate_rejects_unknown_citation_source() -> None:
    citation = Citation(source_id="UNKNOWN", version="1", excerpt="Invented")
    findings = [
        ImpactFinding(
            domain=domain,
        ...
        for domain in ("risk", "operations", "technology")
    ]
    results = evaluate_case(
        complete_request(), findings, [], list(FixtureKnowledgeProvider.documents)
    )
